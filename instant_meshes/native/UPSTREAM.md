...
compiler-verified Apple Silicon and Windows x64 include/source closure used by
this module; legacy build systems, tests, allocators, unused numerical modules,
and unused platform backends are excluded. The only retained Instant Meshes
source edit is that `src/common.h` names the TBB facilities the solver uses
instead of loading TBB's all-in-one header; this does not change the algorithm.
Exact dependency revisions, pruning notes, and all redistribution notices are
recorded in the repository-level `THIRD_PARTY_NOTICES.md`.
